...
| 2026-09 | — | Фиксы P1: M5 MCP читает pi-session JSONL (pi-ledger.ts, source-поаметка); M6 retry только коммита (LLM не повторяется, lastError с результатом); n10 pickReportBody; n4 realpath/symlink в scoped-tools; n5 grep-лимиты; порт референса: auto-resume (length/non-retryable), canSkipObserverWait (fromId>tail), cutoff-snap (закрыта дыра чанка-пересечения), JOURNEY «не конец сессии»; 278 тестов (31 файл) |
